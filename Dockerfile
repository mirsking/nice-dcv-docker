...
RUN apt update && apt install -y --no-install-recommends tar vim wget kmod htop software-properties-common apt-transport-https sudo pciutils ca-certificates xz-utils locales curl mesa-utils libxvmc-dev libxatracker-dev xserver-xorg-core xserver-xorg xserver-xorg-dev xorg x11-utils xauth xinit openbox xfonts-base xterm ubuntu-desktop-minimal gnome-shell gdm3 libglfw3-dev libgles2-mesa-dev libglew-dev glew-utils libvdpau1 libxcb-damage0 libxcb-xtest0 && apt clean && rm -rf /var/lib/apt/lists/*
